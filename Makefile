...
run-x86_64-debug: build-x86_64-debug
	qemu-system-x86_64 -enable-kvm -cpu host,+invtsc -M q35 -m 16G -drive if=pflash,format=raw,readonly=on,file=/usr/share/edk2/ovmf/OVMF_CODE.fd -cdrom catten-x86_64-debug.iso -boot d -serial stdio

run-x86_64-debug-multicore: build-x86_64-debug
	qemu-system-x86_64 -enable-kvm -cpu host,+invtsc -M q35 -smp 16 -cpu host -m 16G -drive if=pflash,format=raw,readonly=on,file=/usr/share/edk2/ovmf/OVMF_CODE.fd -cdrom catten-x86_64-debug.iso -boot d -serial stdio

run-x86_64-debug-numa: build-x86_64-debug
	qemu-system-x86_64 -enable-kvm -M q35 -cpu host -m 16G -drive if=pflash,format=raw,readonly=on,file=/usr/share/edk2/ovmf/OVMF_CODE.fd -cdrom catten-x86_64-debug.iso -boot d -serial stdio -smp 4 -object memory-backend-ram,size=4G,id=m0 -object memory-backend-ram,size=4G,id=m1 -numa node,memdev=m0,cpus=0-1,nodeid=0 -numa node,memdev=m1,cpus=2-3,nodeid=1

run-x86_64-extdb: build-x86_64-debug
	qemu-system-x86_64 -s -S -M q35 -m 16G -drive if=pflash,format=raw,readonly=on,file=/usr/share/edk2/ovmf/OVMF_CODE.fd -cdrom catten-x86_64-debug.iso -boot d -serial stdio -d int -D log.txt -M smm=off

run-x86_64-qdblog: build-x86_64-debug
	qemu-system-x86_64 -M q35 -m 16G -drive if=pflash,format=raw,readonly=on,file=/usr/share/edk2/ovmf/OVMF_CODE.fd -cdrom catten-x86_64-debug.iso -boot d -serial stdio -d int -D log.txt -M smm=off

run-x86_64-log: build-x86_64-debug
	qemu-system-x86_64 -enable-kvm -M q35 -cpu host -m 16G -drive if=pflash,format=raw,readonly=on,file=/usr/share/edk2/ovmf/OVMF_CODE.fd -cdrom catten-x86_64-debug.iso -boot d -serial file:log_x86_64.txt
...
catten-aarch64-debug.iso: build-aarch64-debug
	...
	rm -rf iso_root
run-aarch64-debug: catten-aarch64-debug.iso
	qemu-system-aarch64 -M virt -cpu cortex-a76 -device ramfb -device qemu-xhci -device usb-kbd -m 2G -bios /usr/share/edk2/aarch64/QEMU_EFI.fd -cdrom catten-aarch64-debug.iso -boot d
run-aarch64-log: catten-aarch64-debug.iso
	qemu-system-aarch64 -M virt -cpu cortex-a76 -device ramfb -device qemu-xhci -device usb-kbd -m 2G -bios /usr/share/edk2/aarch64/QEMU_EFI.fd -cdrom catten-aarch64-debug.iso -boot d \
...
catten-aarch64-release.iso: build-aarch64-release
	...
	rm -rf iso_root
run-aarch64-release: catten-aarch64-release.iso
	qemu-system-aarch64 -M virt -cpu cortex-a72 -device ramfb -device qemu-xhci -device usb-kbd -m 2G -bios /usr/share/edk2/aarch64/QEMU_EFI.fd -cdrom catten-aarch64-release.iso -boot d
...
catten-riscv64-debug.iso: build-riscv64-debug
	...
	rm -rf iso_root
run-riscv64-debug: catten-riscv64-debug.iso
	qemu-system-riscv64 -M virt -cpu rv64 \
		-device ramfb -device qemu-xhci -device usb-kbd -m 2G -drive if=pflash,unit=0,format=raw,readonly=on,file=/usr/share/edk2/riscv/RISCV_VIRT_CODE.fd \
		-device virtio-scsi-pci,id=scsi -device scsi-cd,drive=cd0 -drive id=cd0,format=raw,file=catten-riscv64-debug.iso
run-riscv64-debug-log: catten-riscv64-debug.iso
	qemu-system-riscv64 -M virt -cpu rv64 \
		-device ramfb -device qemu-xhci -device usb-kbd -m 2G -drive if=pflash,unit=0,format=raw,readonly=on,file=/usr/share/edk2/riscv/RISCV_VIRT_CODE.fd \
		-device virtio-scsi-pci,id=scsi -device scsi-cd,drive=cd0 -drive id=cd0,format=raw,file=catten-riscv64-debug.iso \
...
catten-riscv64-release.iso: build-riscv64-release
	...
	rm -rf iso_root
run-riscv64-release: catten-riscv64-release.iso
	qemu-system-riscv64 -M virt -cpu rv64 \
		-device ramfb -device qemu-xhci -device usb-kbd -m 2G -drive if=pflash,unit=0,format=raw,readonly=on,file=/usr/share/edk2/riscv/RISCV_VIRT_CODE.fd \
